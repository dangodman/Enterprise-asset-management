  ...
  <div class="container">
    <div>
      ...
      >
        ...
        <vxe-column field="people" title="领用人"></vxe-column>
...
const removeEvent = async (row) => {
  ...
  if (type === "confirm") {
    const data = await acquisitionDelete(row);
    if (data.code === "200") {
...
const submitEvent = async () => {
  ...
  if (selectRow.value) {
    const data = await acquisitionEdit(formData);
    if (data.code === "200") {
    ...
    const data = await acquisitionAdd(formData);
    if (data.code === "200") {
      submitLoading.value = false;
      showEdit.value = false;
